...
ensure_absence_entries_table($pdo);
...
$formData = [
  'user_id' => '',
  'absence_date' => (new DateTimeImmutable('today'))->format('Y-m-d'),
  'hours' => '',
  'note' => '',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!csrf_check($_POST['csrf'] ?? '')) {
    ...
    $id = (int)($_POST['id'] ?? 0);
    if ($id > 0) {
      $stmt = $pdo->prepare('UPDATE absence_entries SET deleted_at = NOW() WHERE id = ? AND deleted_at IS NULL');
      $stmt->execute([$id]);
      header('Location: ' . $base . '/absence.php?msg=deleted');
      exit;
    }
    $message = 'Assenza non valida.';
    $messageType = 'danger';
  } else {
    $formData['user_id'] = (string)($_POST['user_id'] ?? '');
    $formData['absence_date'] = trim((string)($_POST['absence_date'] ?? ''));
    $formData['hours'] = trim(str_replace(',', '.', (string)($_POST['hours'] ?? '')));
    $formData['note'] = trim((string)($_POST['note'] ?? ''));

    $targetUserId = (int)$formData['user_id'];
    $absenceDate = DateTime::createFromFormat('Y-m-d', $formData['absence_date']);
    $hours = filter_var($formData['hours'], FILTER_VALIDATE_FLOAT);

    if ($targetUserId <= 0) {
      $message = 'Seleziona un utente.';
      $messageType = 'danger';
    } elseif (!$absenceDate || $absenceDate->format('Y-m-d') !== $formData['absence_date']) {
      ...
      if (!$checkUser->fetchColumn()) {
        ...
        $stmt = $pdo->prepare('INSERT INTO absence_entries (user_id, absence_date, hours, note, created_by) VALUES (?, ?, ?, ?, ?)');
        $stmt->execute([
          $targetUserId,
        ...
        header('Location: ' . $base . '/absence.php?msg=created');
        exit;
      }
    }
  }
...
$stmt = $pdo->prepare("\n  SELECT a.id, a.absence_date, a.hours, a.note, a.created_at,\n         u.nome, u.cognome, u.email, u.dipartimento,\n         cu.nome AS created_by_nome, cu.cognome AS created_by_cognome, cu.email AS created_by_email\n  FROM absence_entries a\n  JOIN users u ON u.id = a.user_id\n  LEFT JOIN users cu ON cu.id = a.created_by\n  WHERE a.deleted_at IS NULL\n    AND a.absence_date BETWEEN ? AND ?\n  ORDER BY a.absence_date DESC, u.cognome ASC, u.nome ASC, a.id DESC\n");
...
<div class="row g-3">
  <div class="col-12 col-lg-4">
    <div class="card shadow-sm">
      <div class="card-body">
        <h2 class="h6 mb-3">Inserisci ore di assenza</h2>
        <form method="post">
          ...
          <div class="mb-3">
            ...
            <input type="date" name="absence_date" class="form-control" value="<?= e($formData['absence_date']) ?>" required>
          </div>
          <div class="mb-3">
            <label class="form-label">Ore di assenza</label>
          ...
          <button class="btn btn-primary">Salva assenza</button>
        </form>
      </div>
    </div>
  </div>
          ...
          <table class="table table-sm align-middle mb-0">
            ...
            <tbody>
              <?php foreach ($rows as $row): ?>
                <tr>
                  <td><?= e((new DateTime($row['absence_date']))->format('d/m/Y')) ?></td>
                  ...
                  <td class="text-end">
                    <form method="post" class="d-inline" data-confirm-message="Eliminare questa assenza?">
                      <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
              ...
              <?php endforeach; ?>
              <?php if (!$rows): ?>
                <tr><td colspan="6" class="text-center text-muted py-3">Nessuna assenza registrata nel mese selezionato.</td></tr>
              <?php endif; ?>
            </tbody>
          </table>
        </div>
