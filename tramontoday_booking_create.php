...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const fields = ['formula', 'adults_count', 'children_count', 'extra_sunbeds_count', 'discount_percent'];
  ...
  const calculate = function () {
    ...
    const adultPrice = formula === 'giornata_intera' ? Number(form.dataset.adultFull) : Number(form.dataset.adultHalf);
    const childPrice = formula === 'giornata_intera' ? Number(form.dataset.childFull) : Number(form.dataset.childHalf);
    const sunbedPrice = Number(form.dataset.sunbed || '10');
    const total = (readNumber('adults_count') * adultPrice) + (readNumber('children_count') * childPrice) + (readNumber('extra_sunbeds_count') * sunbedPrice);
    const discount = Math.min(readNumber('discount_percent'), 100);
    const finalAmount = Math.max(0, total - (total * discount / 100));
    document.getElementById('total_amount').value = money.format(total);
    document.getElementById('final_amount').value = money.format(finalAmount);
  };
