(function(){
  ...
  var debounce = null;
  var warehouseInput = document.getElementById('scaricoWarehouse');
  ...
  var searchInput = document.getElementById('scaricoSearch');
  var resultsBox = document.getElementById('scaricoResults');
  var cart = document.getElementById('scaricoCart');
  var cartEmpty = document.getElementById('scaricoCartEmpty');
  var hiddenItems = document.getElementById('scaricoHiddenItems');
  var submitBtn = document.getElementById('submitScaricoBtn');
  ...
  function openQtyModal(product){
    ...
    if (qtyModal) {
      ...
      var q = prompt('Quantità da prelevare per ' + product.title);
    ...
    fetch('product_search.php?q=' + encodeURIComponent(q) + '&warehouse=' + encodeURIComponent(warehouse), {credentials:'same-origin'})
      ...
      .then(function(data){
        ...
        resultsBox.innerHTML = data.map(function(r){
          ...
          var badgeClass = num(stock) <= 0 ? 'text-bg-danger' : 'text-bg-light';
          return '<button type="button" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center gap-2"'
            ...
            + '<span class="badge ' + badgeClass + ' flex-shrink-0">' + esc(formatQty(stock)) + '</span>'
            + '</button>';
        }).join('');
      })
      .catch(function(){
  ...
  var form = document.getElementById('scaricoForm');
  if (form) {
    form.addEventListener('submit', function(ev){
      ...
      if (items.length === 0) {
        ev.preventDefault();
        alert('Aggiungi almeno un prodotto prima di inviare l’ordine.');
      }
    });
  }
